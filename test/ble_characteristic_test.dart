...
void main() {
  ...
  group('BleCharacteristic Tests', () {
    ...
    test("service-less isSubscribed checks every service", () async {
      const otherServiceId = "180b";
      final mock = platform as _UniversalBleMock;
      CacheHandler.instance.saveServices(mockDeviceId, [
        mockBleService,
        BleService(otherServiceId, [
      ...
      addTearDown(() => mock.notifierTimer?.cancel());
    });
  });
...
class _UniversalBleMock extends UniversalBlePlatformMock {
  ...
  String _subscriptionKey(String service, String characteristic) =>
  ...
  Future<void> setNotifiable(
    ...
    final subscriptionKey = _subscriptionKey(service, characteristic);
    if (bleInputProperty == BleInputProperty.disabled) {
      _subscriptions[devId]?.remove(subscriptionKey);
      notifierTimer?.cancel();
      notifierTimer = null;
      return;
    }

    _subscriptions.putIfAbsent(devId, () => {}).add(subscriptionKey);
  ...
  Future<bool> isSubscribed(
    ...
    final devId = deviceId.toLowerCase();
    final subscriptionKey = _subscriptionKey(service, characteristic);
    return _subscriptions[devId]?.contains(subscriptionKey) ?? false;
  }

  @override
  Future<List<String>> getSubscribedCharacteristics(String deviceId) async {
    final devId = deviceId.toLowerCase();
    return _subscriptions[devId]
