...
class UniversalBleWriteCommand {
 public:
  // Constructs an object setting all fields.
  explicit UniversalBleWriteCommand(
    const std::string& device_id,
  ...
  int64_t ble_output_property() const;
  void set_ble_output_property(int64_t value_arg);

 private:
  static UniversalBleWriteCommand FromEncodableList(const flutter::EncodableList& list);
  flutter::EncodableList ToEncodableList() const;
  friend class UniversalBlePlatformChannel;
  friend class UniversalBleCallbackChannel;
  friend class PigeonInternalCodecSerializer;
