...
UniversalBleWriteCommand::UniversalBleWriteCommand(
  const std::string& device_id,
 ...
 : device_id_(device_id),
    ...
    value_(value),
    ble_output_property_(ble_output_property) {}

const std::string& UniversalBleWriteCommand::device_id() const {
  return device_id_;
}

void UniversalBleWriteCommand::set_device_id(std::string_view value_arg) {
  device_id_ = value_arg;
}


const std::string& UniversalBleWriteCommand::service() const {
  return service_;
}

void UniversalBleWriteCommand::set_service(std::string_view value_arg) {
  service_ = value_arg;
}


const std::string& UniversalBleWriteCommand::characteristic() const {
  return characteristic_;
}

void UniversalBleWriteCommand::set_characteristic(std::string_view value_arg) {
  characteristic_ = value_arg;
}


const std::vector<uint8_t>& UniversalBleWriteCommand::value() const {
  return value_;
}

void UniversalBleWriteCommand::set_value(const std::vector<uint8_t>& value_arg) {
  value_ = value_arg;
}


int64_t UniversalBleWriteCommand::ble_output_property() const {
  return ble_output_property_;
}

void UniversalBleWriteCommand::set_ble_output_property(int64_t value_arg) {
  ble_output_property_ = value_arg;
}


EncodableList UniversalBleWriteCommand::ToEncodableList() const {
  EncodableList list;
  list.reserve(5);
  list.push_back(EncodableValue(device_id_));
  list.push_back(EncodableValue(service_));
  list.push_back(EncodableValue(characteristic_));
  list.push_back(EncodableValue(value_));
  list.push_back(EncodableValue(ble_output_property_));
  return list;
}

UniversalBleWriteCommand UniversalBleWriteCommand::FromEncodableList(const EncodableList& list) {
  UniversalBleWriteCommand decoded(
    std::get<std::string>(list[0]),
...
EncodableValue PigeonInternalCodecSerializer::ReadValueOfType(
  ...
  switch (type) {
    ...
    case 137: {
        return CustomEncodableValue(UniversalBleWriteCommand::FromEncodableList(std::get<EncodableList>(ReadValue(stream))));
      }
    default:
...
void PigeonInternalCodecSerializer::WriteValue(
  ...
  if (const CustomEncodableValue* custom_value = std::get_if<CustomEncodableValue>(&value)) {
    ...
    if (custom_value->type() == typeid(UniversalBleWriteCommand)) {
      stream->WriteByte(137);
      WriteValue(EncodableValue(std::any_cast<UniversalBleWriteCommand>(*custom_value).ToEncodableList()), stream);
      return;
    }
  }
...
void UniversalBlePlatformChannel::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.batchWrite" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          const auto& encodable_commands_arg = args.at(0);
          if (encodable_commands_arg.IsNull()) {
            reply(WrapError("commands_arg unexpectedly null."));
            return;
          }
          const auto& commands_arg = std::get<EncodableList>(encodable_commands_arg);
          api->BatchWrite(commands_arg, [reply](std::optional<FlutterError>&& output) {
            if (output.has_value()) {
              reply(WrapError(output.value()));
              return;
            }
