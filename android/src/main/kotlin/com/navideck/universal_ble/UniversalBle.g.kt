 ...
 * Generated class from Pigeon that represents data sent in messages.
 */
data class UniversalBleWriteCommand (
  val deviceId: String,
  val service: String,
  val characteristic: String,
  val value: ByteArray,
  val bleOutputProperty: Long
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): UniversalBleWriteCommand {
      val deviceId = pigeonVar_list[0] as String
      ...
      val bleOutputProperty = pigeonVar_list[4] as Long
      return UniversalBleWriteCommand(deviceId, service, characteristic, value, bleOutputProperty)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is UniversalBleWriteCommand) {
      return false
    }
    if (this === other) {
      return true
...
private open class UniversalBlePigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      137.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          UniversalBleWriteCommand.fromList(it)
        }
      }
...
interface UniversalBlePlatformChannel {
   ...
   */
  fun batchWrite(commands: List<UniversalBleWriteCommand>, callback: (Result<Unit>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: UniversalBlePlatformChannel?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.batchWrite$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val commandsArg = args[0] as List<UniversalBleWriteCommand>
            api.batchWrite(commandsArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(UniversalBlePigeonUtils.wrapError(error))
              } else {
