...
///
/// Generated class from Pigeon that represents data sent in messages.
struct UniversalBleWriteCommand: Hashable {
  var deviceId: String
  var service: String
  var characteristic: String
  var value: FlutterStandardTypedData
  var bleOutputProperty: Int64


  // swift-format-ignore: AlwaysUseLowerCamelCase
  static func fromList(_ pigeonVar_list: [Any?]) -> UniversalBleWriteCommand? {
    let deviceId = pigeonVar_list[0] as! String
    ...
    let bleOutputProperty = pigeonVar_list[4] as! Int64
  ...
  static func == (lhs: UniversalBleWriteCommand, rhs: UniversalBleWriteCommand) -> Bool {
    return deepEqualsUniversalBle(lhs.toList(), rhs.toList())  }
  func hash(into hasher: inout Hasher) {
    deepHashUniversalBle(value: toList(), hasher: &hasher)
  }
...
private class UniversalBlePigeonCodecReader: FlutterStandardReader {
  override func readValue(ofType type: UInt8) -> Any? {
    ...
    case 137:
      return UniversalBleWriteCommand.fromList(self.readValue() as! [Any?])
    default:
...
private class UniversalBlePigeonCodecWriter: FlutterStandardWriter {
  override func writeValue(_ value: Any) {
    if let value = value as? UniversalBleErrorCode {
      ...
      super.writeByte(136)
      super.writeValue(value.toList())
    } else if let value = value as? UniversalBleWriteCommand {
      super.writeByte(137)
      super.writeValue(value.toList())
    } else {
...
class UniversalBlePlatformChannelSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: UniversalBlePlatformChannel?, messageChannelSuffix: String = "") {
    ...
    /// All writes are executed simultaneously without waiting for individual completions
    /// Returns when all writes have completed (or failed)
    let batchWriteChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.universal_ble.UniversalBlePlatformChannel.batchWrite\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      batchWriteChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let commandsArg = args[0] as! [UniversalBleWriteCommand]
        api.batchWrite(commands: commandsArg) { result in
          switch result {
          case .success:
            reply(wrapResult(nil))
          case .failure(let error):
      ...
      batchWriteChannel.setMessageHandler(nil)
    }
